...
function renderChordSvg(symbol, voicing, opts = {}) {
  ...
  let marks = "";
  if (base === 1) {
    marks += `<rect x="${padL - 1}" y="${padT - 3}" width="${gridW + 2}" height="3.5" fill="#0071e3"/>`;
  } else {
    marks += `<text x="${padL - 5}" y="${padT + gridH / 8 + 3}" fill="#86868b" font-size="9" font-family="Manrope,system-ui,sans-serif" text-anchor="end">${base}fr</text>`;
  }

  for (const x of stringXs) {
    marks += `<line x1="${x}" y1="${padT}" x2="${x}" y2="${padT + gridH}" stroke="#0071e3" stroke-width="1" opacity="0.45"/>`;
  }
  for (const y of fretYs) {
    marks += `<line x1="${padL}" y1="${y}" x2="${padL + gridW}" y2="${y}" stroke="#0071e3" stroke-width="1" opacity="0.35"/>`;
  }
  ...
  if (barreFret !== null) {
    ...
    marks += `<rect x="${stringXs[barreFrom] - 5}" y="${y - 5}" width="${stringXs[barreTo] - stringXs[barreFrom] + 10}" height="10" rx="5" fill="#0071e3"/>`;
  }
  ...
  for (let s = 0; s < 6; s++) {
    ...
    const x = stringXs[s];
    if (f === FRET_MUTE) {
      marks += `<text x="${x}" y="${padT - 10}" fill="#86868b" font-size="11" text-anchor="middle" font-family="Manrope,system-ui,sans-serif">×</text>`;
    } else if (f === FRET_OPEN) {
      marks += `<circle cx="${x}" cy="${padT - 12}" r="3.8" fill="none" stroke="#0071e3" stroke-width="1.4"/>`;
    } else {
      const relF = f - base + 1;
      if (relF < 1 || relF > showFrets) continue;
      if (barreFret === relF && s > barreFrom && s < barreTo) continue;
      const y = padT + ((relF - 0.5) * gridH) / 4;
      marks += `<circle cx="${x}" cy="${y}" r="5.2" fill="#0071e3" stroke="#0077ed" stroke-width="1"/>`;
    }
  }
  ...
  return `
    <figure class="chord-diag">
      ...
      <svg viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" role="img" aria-label="Аппликатура ${symbol}">
        <text x="${w / 2}" y="14" text-anchor="middle" fill="#1d1d1f" font-size="12" font-weight="600" font-family="Manrope,system-ui,sans-serif">${symbol}</text>
        ${marks}
      </svg>
...
function renderPianoSvg(item, opts = {}) {
  ...
  whites.forEach((m, i) => {
    ...
    svg += `<rect x="${x}" y="${padT}" width="${whiteW - 1.2}" height="${whiteH}" rx="1.5" fill="${on ? "#0071e3" : "#ffffff"}" stroke="rgba(0,0,0,0.18)" stroke-width="1"/>`;
    if (on && fingerOf[m]) {
      svg += `<text x="${x + (whiteW - 1.2) / 2}" y="${padT + whiteH - 8}" text-anchor="middle" fill="#ffffff" font-size="10" font-weight="700" font-family="Manrope,system-ui,sans-serif">${fingerOf[m]}</text>`;
    }
  });
  ...
  whites.forEach((m, i) => {
    ...
    if (nextBlack <= end && isBlack(nextBlack)) {
      ...
      svg += `<rect x="${x}" y="${padT}" width="${blackW}" height="${blackH}" rx="1" fill="${on ? "#0071e3" : "#1c1917"}" stroke="#0d0a08" stroke-width="0.5"/>`;
      if (on && fingerOf[nextBlack]) {
        svg += `<text x="${x + blackW / 2}" y="${padT + blackH - 6}" text-anchor="middle" fill="#ffffff" font-size="9" font-weight="700" font-family="Manrope,system-ui,sans-serif">${fingerOf[nextBlack]}</text>`;
      }
    }
  });
  ...
  return `
    <figure class="chord-diag chord-diag-piano">
      ...
      <svg viewBox="0 0 ${w} ${h}" width="${Math.min(w, 200)}" height="${h}" role="img" aria-label="Рояль ${item.symbol}">
        <text x="${w / 2}" y="14" text-anchor="middle" fill="#1d1d1f" font-size="12" font-weight="600" font-family="Manrope,system-ui,sans-serif">${item.symbol}</text>
        ${svg}
      </svg>
...
function renderScaleBoxSvg(pcs, rootPc, baseFret, opts = {}) {
  ...
  if (baseFret <= 1) {
    ...
    marks += `<text x="${padL - 5}" y="${padT + gridH / 10 + 3}" fill="#86868b" font-size="9" font-family="Manrope,system-ui,sans-serif" text-anchor="end">${baseFret}fr</text>`;
  }
  ...
  for (let s = 0; s < 6; s++) {
    for (let fretNum = fromFret; fretNum <= toFret; fretNum++) {
      ...
      if (fretNum === 0) {
        marks += `<circle cx="${x}" cy="${padT - 11}" r="${isRoot ? 4.4 : 3.4}" fill="${isRoot ? "#0071e3" : "none"}" stroke="#0071e3" stroke-width="1.4"/>`;
        continue;
      }
      ...
      if (isRoot) {
        marks += `<circle cx="${x}" cy="${y}" r="6" fill="#0071e3" stroke="#0077ed" stroke-width="1.2"/>`;
        marks += `<text x="${x}" y="${y + 3.2}" text-anchor="middle" fill="#ffffff" font-size="8" font-weight="700" font-family="Manrope,system-ui,sans-serif">R</text>`;
      } else {
        marks += `<circle cx="${x}" cy="${y}" r="4.6" fill="#f5f5f7" stroke="#0071e3" stroke-width="1.1"/>`;
      }
    }
  }
  ...
  return `
    <figure class="chord-diag scale-box-diag">
      <svg viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" role="img" aria-label="Бокс ${title}">
        <text x="${w / 2}" y="14" text-anchor="middle" fill="#1d1d1f" font-size="11" font-weight="600" font-family="Manrope,system-ui,sans-serif">${title}</text>
        ${marks}
      </svg>
